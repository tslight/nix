...
cp /etc/nixos/hardware-configuration.nix ./nixos/hosts/$NEW_HOSTNAME.nix
#+end_src

- Add/Edit the following at the top of =./nixos/hosts/$NEW_HOSTNAME.nix=,
  making sure to remove the now redundant =nixpkgs.hostPlatform= line at the
  bottom of the file:
...
{ lib, modulesPath, host, system, ... }: {
  ...
  # REMOVE THIS LINE:
  nixpkgs.hostPlatform = "your-machine-architecture"
}
#+end_src
...
- Run =sudo nixos-rebuild switch --flake ./nixos/#"$NEW_HOSTNAME"=.
- Reboot, to ensure we can successfully boot from the flake's derivation.
- Run =make clean= to delete all the old channel cruft.
- Run =make setup= to upload newly created SSH keys to GitHub and then change
  our git remote.
...
| =tsot=         | Apple MacBook Air 7,2          | 13”    | NixOS |
| =genesis=      | Apple MacBook Air 6,1          | 11”    | NixOS |
| =hexley=       | Apple MacBook Pro M1           | 14”    | macOS |
| =porridge=     | Pine64 Pinebook Pro            | 14”    | NixOS |
| =sunnyRainbow= | Lenovo ThinkPad X131e          | 11”    | NixOS |
